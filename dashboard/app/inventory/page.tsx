  ...
  return (
    <div className="flex h-full flex-col bg-paper">
      ...
      <div className="flex flex-1 flex-col overflow-hidden lg:flex-row">
        <section className="flex min-w-0 flex-1 flex-col overflow-hidden">
          <div className="flex flex-wrap items-center gap-2 border-b border-line bg-card px-4 py-3">
            <input
              ...
              className="min-w-40 flex-1 rounded-lg border border-line px-3 py-1.5 text-sm"
            />
            <label className="flex items-center gap-1.5 text-sm text-soy">
              <input
                type="checkbox"
                checked={trackedOnly}
            ...
            <button
              ...
              className="rounded-lg border border-line px-3 py-1.5 text-sm hover:bg-paper disabled:opacity-50"
            >
              Recheck totals
            </button>
          </div>

          <div className="flex gap-3 border-b border-line bg-card px-4 py-2 text-xs text-soy">
            <span>{tracked.length} tracked</span>
            {out.length > 0 && (
              <span className="font-medium text-chilli">{out.length} out of stock</span>
            )}
            {low.length > 0 && (
          ...
          {error && <p className="bg-chilli-wash px-4 py-2 text-sm text-chilli-dark">{error}</p>}
          {notice && <p className="bg-scallion-wash px-4 py-2 text-sm text-scallion">{notice}</p>}

          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <p className="px-4 py-6 text-sm text-soy">Loading…</p>
            ) : (
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-paper text-left text-xs uppercase text-soy">
                  <tr>
                ...
                <tbody>
                  ...
                  {visible.length === 0 && (
                    <tr>
                      <td colSpan={4} className="px-4 py-6 text-sm text-soy">
        ...
        <aside className="flex w-full flex-col border-t border-line bg-card lg:w-96 lg:border-l lg:border-t-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {selected && movements.length === 0 && (
              <p className="px-4 py-4 text-sm text-soy">
              ...
              {movements.map((m) => (
                <li key={m.id} className="px-4 py-2.5 text-sm">
                  <div className="flex items-baseline justify-between gap-2">
                    ...
                    <span
                      className={`font-mono ${m.delta > 0 ? 'text-emerald-700' : 'text-chilli'}`}
                    >
                      {m.delta > 0 ? `+${m.delta}` : m.delta}
                    </span>
                  </div>
                  <div className="text-xs text-soy">
                    {new Date(m.created_at).toLocaleString()} · {m.created_by}
                  </div>
                  {m.note && <div className="mt-0.5 text-xs text-soy">{m.note}</div>}
                </li>
              ))}
            </ul>
          </div>
...
function StockRow({
  ...
  return (
    <tr className={`border-b border-line ${selected ? 'bg-paper' : 'hover:bg-black/[0.02]'}`}>
      ...
      <td className="px-2 py-2">
        <button
          ...
          className={`rounded-full px-2 py-0.5 text-xs font-medium ${
            item.track_stock
              ? 'bg-scallion-wash text-scallion ring-1 ring-emerald-200'
              : 'bg-paper text-soy'
          }`}
        >
